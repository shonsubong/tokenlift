...
```
[탐색] codebase-memory-mcp 그래프 (입력 토큰↓)
[생성] 온프렘 위임: V100(coder) → H200 → GLM-5.2(oracle, vLLM) — 사내망 직결(보안 예외)
[판단] Claude(Bedrock) — 설계·보안·최종 검토 — NemoClaw 게이트웨이 경유(보안 필터)
[보안] 민감 폴더는 Claude Code permissions.deny 로 애초에 읽기 차단(유출 원천 봉쇄)
```
...
> [16. GLM 온프렘 셋업](./16-glm-multiquant-team.md)
...
| 역할 | 1회 작업 | 산출물 |
|---|---|---|
| **서버 관리자** | 온프렘 LLM 서빙(GLM-5.2 vLLM / V100·H200 Ollama), (선택) NemoClaw NIM | 사내망 `/v1` 엔드포인트 + 사용자 토큰 |
...
하드웨어로 양자화를 고른다(상세 [16](./16-glm-multiquant-team.md)):

| HW | 양자화(model id) | 명령 |
|---|---|---|
| **Blackwell**(B200/B300) | NVIDIA 공식 NVFP4 `glm-5.2-nvfp4` | `PROFILE=nvfp4 bash scripts/run-glm-vllm.sh` |
| **H200**(Hopper) | Z.ai 공식 FP8 `glm-5.2-fp8` | `PROFILE=fp8 bash scripts/run-glm-vllm.sh` |
| 저VRAM/CPU offload | Unsloth GGUF `glm-5.2-q4/q2` | `bash scripts/run-glm-fleet.sh start` |
...
③ 무거운 생성/반복 작업? ── yes ──▶ 역할/티어 결정 (tokenlift route / 휴리스틱)
   ...
   │                                    ▼  결과(코드/텍스트) stdout 반환
   ▼                                    │
⑤ Claude(Bedrock) 판단·검토 ◀───────────┘
   └ Claude Code 의 모델 호출 → ANTHROPIC_BEDROCK_BASE_URL = NemoClaw 게이트웨이
        → 게이트웨이가 PII redaction/정책 필터 적용 → AWS Bedrock
...
- 보안 → [15. NemoClaw 보안(Windows/WSL2)](./15-nemoclaw-windows-security.md)
