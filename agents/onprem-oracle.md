...
```bash
tokenlift warmup --provider onprem-h200 -m deepseek-r1:70b
tokenlift gen "락-프리 큐 알고리즘 구현(요구: ...)" --role oracle -o src/queue.ts
tokenlift refactor "도메인 모델 전반을 이벤트 소싱으로" --role oracle -f src/domain.ts
```
> 체인: oracle = onprem-h200 → onprem-v100 → claude. H200 연결 실패 시 자동 강등된다.
