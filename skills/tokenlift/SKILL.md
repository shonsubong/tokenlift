...
| **coder** | **V100 → 로컬ollama → H200** | 대량·정형 생성(최저가) | `--role coder` |
| **oracle** | **H200 → V100 → claude** | 어려운 추론·대형 생성 | `--role oracle` |
| **reviewer** | claude | 보안·최종 검토·의사결정 | (Claude 자신) |
...
| **아키텍처·시스템 설계, API/인터페이스 설계** | **Claude** | 고난도 판단(단, 현황 파악은 그래프로) |
